...
- **Superseded by:** none (amended by
  [ADR 0013](0013-native-mac-app-is-the-coachs-editing-desk.md): the mapping's second
  implementation is now the native Mac app's Swift port, held to the TypeScript by the golden
  vectors in `contracts/`; `hockey-video-pipeline` is retired)
