/**
 * Map ready team clips into the player's {@link PlaylistItem} list (P0-10). This
 * is the pure, server-side display layer: it builds each item's German label and
 * resolves the stored NAS `output_path` into a loadable URL, so the login-free
 * client only ever receives display-ready strings, never a tag-type key or a raw
 * file path.
 */
import { teamShareContent } from "./content";
import type { TeamClipRow } from "./queries";
...
function buildSubtitle(row: TeamClipRow): string {
  const parts = [
    row.gameTitle,
    row.gameOpponent
      ? `${teamShareContent.opponentPrefix} ${row.gameOpponent}`
      : null,
