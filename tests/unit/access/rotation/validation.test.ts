...
import { isValidPlayerId } from "@/features/access/rotation";

describe("isValidPlayerId (rotation)", () => {
  ...
  it("rejects strings that are not a UUID", () => {
    ...
    expect(isValidPlayerId("3f2504e0-4f89-41d3-9a0c-0305e82c3301x")).toBe(
      false,
    );
  });
  ...
  it("rejects non-string input a client might smuggle in", () => {
    ...
    expect(isValidPlayerId(["3f2504e0-4f89-41d3-9a0c-0305e82c3301"])).toBe(
      false,
    );
  });
});
