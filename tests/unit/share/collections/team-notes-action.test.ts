...
// without a coach session or with an invalid text, and that the team notes
// only ever go to the team notes store.
const { getCurrentCoach, saveTeamNotes, savePresenterNotes, revalidatePath } =
  vi.hoisted(() => ({
...
vi.mock("@/features/share/collections/team-notes", () => ({ saveTeamNotes }));
vi.mock("@/features/share/collections/presenter-notes", () => ({
  savePresenterNotes,
}));
vi.mock("next/cache", () => ({ revalidatePath }));

import { saveTeamNotesAction } from "@/features/share/collections/actions";
...
function form(
  ...
  data.set(TEAM_INTRO_FIELD, intro);
  data.set(`${TEAM_CLIP_NOTE_FIELD_PREFIX}${CLIP_ID}`, clipNote);
  return data;
}

function save(data: FormData) {
  return saveTeamNotesAction(collectionMutationInitialState, data);
}

beforeEach(() => {
  vi.clearAllMocks();
  getCurrentCoach.mockResolvedValue(COACH);
  saveTeamNotes.mockResolvedValue(true);
});

describe("saveTeamNotesAction", () => {
  it("stores the validated texts and refreshes the detail page", async () => {
    const result = await save(form(" Heute: Ecken ", "Läufer beachten"));

    expect(result).toEqual({ status: "success" });
    expect(saveTeamNotes).toHaveBeenCalledWith(COLLECTION_ID, {
      collection: "Heute: Ecken",
  ...
  it("stores only the team fields, never as presenter notes", async () => {
    const data = form("Heute: Ecken");
    data.set(COLLECTION_NOTE_FIELD, "Privat");
    data.set(`${CLIP_NOTE_FIELD_PREFIX}${CLIP_ID}`, "Privat");

    await save(data);

    expect(saveTeamNotes).toHaveBeenCalledWith(COLLECTION_ID, {
      collection: "Heute: Ecken",
      clips: new Map([[CLIP_ID, null]]),
    });
  ...
  it("stores nothing without a coach session", async () => {
    ...
    const result = await save(form("Heute"));

    expect(result).toEqual({ status: "error", error: errors.unauthorized });
    expect(saveTeamNotes).not.toHaveBeenCalled();
  });

  it("rejects a malformed collection id before any query", async () => {
    const result = await save(form("Heute", "", "not-a-uuid"));

    expect(result).toEqual({ status: "error", error: errors.invalidId });
    expect(saveTeamNotes).not.toHaveBeenCalled();
  });

  it("rejects the whole save when one text is too long", async () => {
    const result = await save(
      form("Heute", "a".repeat(MAX_TEAM_NOTE_LENGTH + 1)),
    );

    expect(result).toEqual({ status: "error", error: errors.invalidTeamNote });
    expect(saveTeamNotes).not.toHaveBeenCalled();
  });

  it("reports an unknown collection", async () => {
    saveTeamNotes.mockResolvedValue(false);
  ...
  it("reports a failing store without leaking the cause", async () => {
    saveTeamNotes.mockRejectedValue(new Error("db down"));
